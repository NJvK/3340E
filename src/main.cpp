...
inline bool eyeSeesBlue() {
  if (colorSwapRedMode == false) return true;
  return false;
}
inline bool eyeSeesRed() {
  if (colorSwapRedMode == true) return true;
  return false;
}
...
inline const char* autonName(int sel) {
  switch (sel) {
    case 0: return "RED"; //LEFT RED
    case 1: return "BLUE"; //LEFT BLUE
    case 2: return "SKILLS";
    case 3:return "RED"; //RIGHT RED
    case 4: return "BLUE"; //RIGHT BLUE
  }
  return "—";
}
...
inline void drawUI(const Button btns[], int n, int selectedId, double heading) {
  ...
  for (int i = 0; i < n; ++i) drawButton(btns[i], btns[i].id == selectedId);
    ...
    if (Brain.Screen.pressing()) {
      ...
      for (int i = 0; i < BTN_COUNT; ++i) {
        if (inRect(x, y, buttons[i])) {
          ...
          drawUI(buttons, BTN_COUNT, current_auton_selection, chassis.get_absolute_heading());
          while (Brain.Screen.pressing()) wait(5, msec); // debounce
          break;
        }
      }
    }
